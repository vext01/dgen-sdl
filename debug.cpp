...
// simple wrapper around strtoll with error check
// -1 on error
static int debug_strtoll(const char *str, long long *ret)
{
	char			*end = NULL;

	errno = 0;
	*ret = strtoll(str, &end, 0);
...
void debug_init()
{
	...
	memset(debug_bp_m68k, 0, sizeof(struct dgen_bp) * MAX_BREAKPOINTS);
	memset(debug_wp_m68k, 0, sizeof(struct dgen_wp) * MAX_WATCHPOINTS);
...
int md::debug_cmd_watch(int n_args, char **args)
{
	...
	switch (n_args) {
	case 2:
		if ((debug_strtoll(args[1], &len)) < 0) {
			printf("length malformed: %s\n", args[1]);
			return (1);
		}
		// fallthru
	case 1:
		if ((debug_strtoll(args[0], &start)) < 0) {
...
{
	long long		addr, len = DEBUG_DFLT_MEMDUMP_LEN;
	...
	case 1: /* default length */
		if ((debug_strtoll(args[0], &addr)) < 0) {
...
{
	long long		addr = m68k_state.pc;
	long long		length = DEBUG_DFLT_DASM_LEN;
	...
	switch (n_args) {
		case 0:
			...
			break;
		case 2:
			if ((debug_strtoll(args[1], &length)) < 0) {
				printf("length malformed: %s\n", args[1]);
				return (1);
			}
			// fallthru
...
int md::debug_cmd_minus_watch(int n_args, char **args)
{
	...
	if (args[0][0] == '#') { // remove by index
		...
		if ((debug_strtoll(args[0]+1, &num)) < 0) {
			printf("address malformed: %s\n", args[0]);
			return (1);
		}
		index = num;
...
void md::debug_enter()
{
	...
	pd_message((const char *)"Debug trap.");
